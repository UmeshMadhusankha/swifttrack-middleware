...
/** Declares this adapter's billing queue and its cancellation queue. */
@Configuration
public class RabbitConfig {
    ...
    public Queue billingQueue() {
        return QueueBuilder.durable(MessagingConstants.BILLING_QUEUE).build();
    }

    @Bean
    public Binding billingBinding(Queue billingQueue, TopicExchange commandsExchange) {
        return BindingBuilder.bind(billingQueue).to(commandsExchange).with(MessagingConstants.BILLING_KEY);
    }
